...
@[to_additive (attr := simp)]
lemma coeff_mapCoeffAlgHom (f : A →ₐ[R] B) (x : A[M]) (m : M) :
    (mapCoeffAlgHom M f x).coeff m = f (x.coeff m) := by simp [mapRangeAlgHom]

@[deprecated (since := "2026-03-20")] alias mapRangeAlgHom_apply := coeff_mapCoeffAlgHom
...
@[to_additive (attr := simp)]
lemma mapCoeffAlgEquiv_trans (e₁ : A ≃ₐ[R] B) (e₂ : B ≃ₐ[R] C) :
