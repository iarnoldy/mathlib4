...
protected noncomputable def opRingEquiv : R[M]ᵐᵒᵖ ≃+* Rᵐᵒᵖ[Mᵐᵒᵖ] where
  toAddEquiv :=
    opAddEquiv.symm.trans <| (mapDomainAddEquiv _ opEquiv).trans <| mapCoeffAddEquiv _ opAddEquiv
  map_mul' := by
    classical
    simp [coeff_mul, MonoidAlgebra.ext_iff, Finsupp.ext_iff, ← MulOpposite.unop_inj,
      unop_finsuppSum, sum_mapRange_index, apply_ite unop, mapCoeffAddEquiv]
