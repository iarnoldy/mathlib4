...
theorem mapCoeff_eq_map {R S : Type*} [CommSemiring R] [CommSemiring S] (p : MvPolynomial σ R)
    (f : R →+* S) : p.mapCoeff f = map f p := rfl
...
@[simp] lemma restrictSupport_zero : restrictSupport R (0 : Set (σ →₀ ℕ)) = 1 := by
  ...
  · rintro _ ⟨x, rfl⟩
    ...
    simp [AddMonoidAlgebra.one_def, Finsupp.single_apply]
